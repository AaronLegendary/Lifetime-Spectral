...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Bro you suck!', 0.2], //From 0% to 19%
		['Dammit', 0.4], //From 20% to 39%
		['Stupid!', 0.5], //From 40% to 49%
		['Oof', 0.6], //From 50% to 59%
		['huh', 0.69], //From 60% to 68%
		['Cool', 0.7], //69%
		['Awesome!', 0.8], //From 70% to 79%
		['keep it up!', 0.9], //From 80% to 89%
		['Holy crap!', 1], //From 90% to 99%
		['OOOOOOH!!!!!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	var middletownTvscreen:FlxTypedGroup<BGSprite>;
	var cameos:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'middletown':
				var bgSky:BGSprite = new BGSprite('middletown/Sky', 100, -855, 1, 1);
				add(bgSky);

				var backtown:BGSprite = new BGSprite('middletown/BackTown', -1270, -960);
				backtown.setGraphicSize(Std.int(backtown.width * 1.11));
				add(backtown);

				var airship:BGSprite = new BGSprite('middletown/Airship', 1175, -552, 1, 1);
				airship.setGraphicSize(Std.int(airship.width * 1.14));
				add(airship);

				middletownTvscreen = new FlxTypedGroup<BGSprite>();
				add(middletownTvscreen);

				for (i in 0...5)
				{
					var tvscreen:BGSprite = new BGSprite('middletown/tvscreen' + i, -1495, -1045, 1, 1);
					tvscreen.visible = false;
					tvscreen.setGraphicSize(Std.int(tvscreen.width * 1.11));
					tvscreen.updateHitbox();
					middletownTvscreen.add(tvscreen);
				}

				var streets:BGSprite = new BGSprite('middletown/Streets', -890, 190);
				streets.setGraphicSize(Std.int(streets.width * 1.35));
				add(streets);

				cameos = new BGSprite('middletown/Cameos', -1050, 70, 1, 1, ['Cameos bop']);
				cameos.setGraphicSize(Std.int(cameos.width * 1.15));
				cameos.updateHitbox();
				add(cameos);
		}
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "HI CHEATER!", 32);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week2_nomiss', 'week3_nomiss', 'week4_nomiss',
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'middletown':
				if (curBeat % 4 == 0)
				{
					middletownTvscreen.forEach(function(tvscreen:BGSprite)
					{
						tvscreen.visible = false;
					});

					curLight = FlxG.random.int(4, middletownTvscreen.length - 5, [curLight]);

					middletownTvscreen.members[curLight].visible = true;
					middletownTvscreen.members[curLight].alpha = 6;
				}
